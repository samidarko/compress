...
object CommandLine {
  ...
  def getParser = new scopt.OptionParser[Config]("compressor") {
    head("Compressor", currentVersion)
    ...
    cmd("compress") action ((_, c) => c.copy(mode = "compress")) children (
      opt[Int]('s', "size") required() valueName "<MB>" action((x, c) => c.copy(fileSize = x)) text "Maximum compressed size per file expressed in MB"
